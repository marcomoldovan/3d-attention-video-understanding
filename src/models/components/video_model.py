...
class SpatioTemporalTransformer3D(nn.Module):
    """
...
if __name__ == "__main__":
    # _ = SpatioTemporalTransformer3D(patch_size=[4, 4, 4], embed_dim=96, depths=[6, 3, 2], num_heads=[12, 6, 4], window_size=[7, 7, 7]) #! leave this line, delete the following after testing
    model = SpatioTemporalTransformer3D(patch_size=[4, 4, 4], embed_dim=64, depths=[2, 2, 2], num_heads=[8, 8, 8], window_size=[6, 6, 6]) # num_heads must be powers of two (or do they have to just divide the embed_dim?)

    from torchinfo import summary
    model.eval()
    input = torch.randn(32, 3, 12, 64, 64)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    input = input.to(device)
    
    summary(model, input_size=(32, 3, 12, 64, 64))
    output = model(input)
    print(output.shape)